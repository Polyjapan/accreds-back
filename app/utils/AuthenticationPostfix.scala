...
object AuthenticationPostfix {
  ...
  object AuthorizationHandler {
    ...
    def ensuringGroup(group: String): AuthorizationHandler = ensuringAuthentication.andAlso((user: Option[UserSession]) => user.get match {
      case AdminUser(_, groups, _) =>groups.contains(group)
      case _ => false
    })
    def ensuringAdmin: AuthorizationHandler = ensuringAuthentication.andAlso((user: Option[UserSession]) => user.get.isInstanceOf[AdminUser])
    def ensuringStaff: AuthorizationHandler = ensuringAuthentication.andAlso((user: Option[UserSession]) => user.get.isInstanceOf[StaffUser])
  }
  ...
  implicit class AuthenticationPostfix[T](action: Action[T]) {
    ...
    def requiresAdmin(implicit conf: Configuration, clock: Clock): Action[T] = AuthenticationAction(action, AuthorizationHandler.ensuringAdmin)

    def requiresStaff(implicit conf: Configuration, clock: Clock): Action[T] = AuthenticationAction(action, AuthorizationHandler.ensuringStaff)
  }
